package co.mike.shareme.widget;
...
public class CircularImageView extends AppCompatImageView {
  ...
  private void init(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    if (attributes.getBoolean(R.styleable.CircularImageView_civ_border, true)) {
      ...
      setBorderWidth(attributes.getDimension(R.styleable.CircularImageView_civ_border_width,
          defaultBorderSize));
      setBorderColor(
  ...
  public void addShadow() {
    if (shadowRadius == 0) shadowRadius = DEFAULT_SHADOW_RADIUS;
  ...
  private void loadBitmap() {
    if (this.drawable == getDrawable()) return;
  ...
  private Bitmap cropBitmap(Bitmap bitmap) {
    ...
    if (bitmap.getWidth() >= bitmap.getHeight()) {
      bmp = Bitmap.createBitmap(bitmap, bitmap.getWidth() / 2 - bitmap.getHeight() / 2, 0,
          bitmap.getHeight(), bitmap.getHeight());
    } else {
      bmp = Bitmap.createBitmap(bitmap, 0, bitmap.getHeight() / 2 - bitmap.getWidth() / 2,
